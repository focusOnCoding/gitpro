...
<# The first thing this command does is pull down the info/refs file. This file is written by the updateserver-info command, which is why you need to enable that as a post-receive hook in order for the
...
<# Uploading Data
To upload data to a remote process, Git uses the send-pack and receive-pack processes. The sendpack process runs on the client and connects to a receive-pack process on the remote side.#>
